...
class GoogleSheetInReadDataFrameSpec
  ...
  "Google sheet can be used as input" in {
    ...
    val dataframe = read("1vI74zw07kIIfmvmciEbWkwDnGG_DI66ErBYaE-o02MY", serviceAccountCredentials)
    dataframe.schema.get.fields.length should be  > 1
  }

  private def read(googleSheetId: String, serviceAccountCredentialsJson: String): DataFrame = {
    val readDF = new ReadDataFrame()
      .setStorageType(
        new InputStorageTypeChoice.GoogleSheet()
          .setGoogleSheetId(googleSheetId)
          .setGoogleServiceAccountCredentials(serviceAccountCredentialsJson)
      )
