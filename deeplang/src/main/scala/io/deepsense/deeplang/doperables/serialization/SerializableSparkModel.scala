...
class SerializableSparkModel[M <: Model[M]](val model: ml.Model[M])
  extends Model[M]
  with MLWritable {

  override def copy(extra: ParamMap): M =
    new SerializableSparkModel(model.copy(extra)).asInstanceOf[M]

  override def write: MLWriter = new DefaultMLWriter(this)

  override def transform(dataset: DataFrame): DataFrame = model.transform(dataset)
