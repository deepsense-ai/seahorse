...
import io.deepsense.deeplang.parameters.{SingleColumnSelectorParameter, ParametersSchema}
...
class TrainRegressor extends DOperation2To1[Trainable, DataFrame, Scorable] {
  ...
  val targetColumnField = "target column"
  ...
  override protected def _execute(
    ...
    trainable.train(context)("abc")(dataframe)  // TODO use target column
  }
  ...
  override protected def _inferKnowledge(context: InferContext)(
    ...
    DKnowledge(
      for (trainable <- trainableKnowledge.types)
        yield trainable.train.infer(context)("abc")(dataframeKnowledge))  // TODO use target column
  }
}
