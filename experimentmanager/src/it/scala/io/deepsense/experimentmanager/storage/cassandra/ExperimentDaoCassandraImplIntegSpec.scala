...
import io.deepsense.experimentmanager.ExperimentsTableCreator
import io.deepsense.commons.cassandra.CassandraTestSupport
import io.deepsense.graph.{Endpoint, Edge, Node, Graph}
...
class ExperimentDaoCassandraImplIntegSpec
  ...
  var experimentsDao : ExperimentDaoCassandraImpl = _
  val catalog = mock[DOperationsCatalog]
  val graphReader: GraphReader = new GraphReader(catalog)
  val rowMapper = new ExperimentRowMapper(graphReader)
  ...
  before {
    ...
    experimentsDao = new ExperimentDaoCassandraImpl(cassandraTableName, session, rowMapper)
  }
  ...
  private def withStoredExperiments(storedExperiments: Set[Experiment])(testCode: => Any): Unit = {
  ...
  def createExperiment(
      ...
      name: String = "test",
      state: Experiment.State = Experiment.State.draft) : Experiment = {
    val now = DateTimeConverter.now
    new Experiment(Experiment.Id.randomId, tenant, name, graph, now, now, "description", state)
  }
