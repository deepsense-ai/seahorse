...
case class EnvelopeJsonWriter[T : JsonWriter](private val label: String)
  ...
  override def write(e: Envelope[T]) = EnvelopeJsonConversions.write[T](label)(e)
}
...
case class EnvelopeJsonReader[T : JsonReader](private val label: String)
  ...
  override def read(json: JsValue) = EnvelopeJsonConversions.read[T](label)(json)
}
...
private object EnvelopeJsonConversions {
  def read[T : JsonReader](label: String)(json: JsValue) = json match {
  ...
  def write[T : JsonWriter](label: String)(e: Envelope[T]) =
