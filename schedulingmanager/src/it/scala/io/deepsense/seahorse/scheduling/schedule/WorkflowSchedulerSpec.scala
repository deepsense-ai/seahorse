...
class WorkflowSchedulerSpec extends WordSpec with Matchers {
  "WorkflowScheduler" should {
    "schedule and unschedule workflow execution" in {
      ...
      TestJob.executions.foreach { case (executionWorkflowId, executionEmail) =>
...
private[scheduling] class TestJob extends WorkflowJob {
  override def runWorkflow(workflowId: String, email: String): Future[Unit] = {
    TestJob.executions = TestJob.executions :+ (workflowId, email)
    Future.successful(())
  }
}
private[scheduling] object TestJob {
  var executions: Seq[(String, String)] = Seq.empty
}
