...
class RunningWorkflowsActorSpec
  ...
  val experiment = Workflow(Workflow.Id.randomId, "B", "Experiment",
  ...
  trait TestCase {
    ...
    val actorRef: TestActorRef[RunningWorkflowsActor] = TestActorRef(
      Props(new RunningWorkflowsActor(
    ...
    def launch(experiments: Set[Workflow]): Unit = {
  ...
  "RunningExperimentsActor" should {
    ...
    "not launch experiment and respond Failure" when {
      "received Launch on running experiment" in new TestCase {
        probe.send(actorRef, Launch(experiment))
        probe.expectMsgClass(classOf[Success[Workflow]])
        probe.send(actorRef, Launch(experiment))
        val failure = probe.expectMsgClass(classOf[Failure[Workflow]])
    ...
    "abort experiment and respond Success" when {
      "received Abort on running experiment" in new TestCase {
        ...
        val success = probe.expectMsgClass(classOf[Success[Workflow]])
    ...
    "not abort experiment and respond Failure" when {
      "received Abort on not running experiment" in new TestCase {
        ...
        probe.send(actorRef, Abort(experiment.id))
        val failure = probe.expectMsgClass(classOf[Failure[Workflow]])
        failure.exception shouldBe a [WorkflowNotRunningException]
      }
      "received Abort on not existing experiment" in new TestCase {
        probe.send(actorRef, Abort(experiment.id))
        val failure = probe.expectMsgClass(classOf[Failure[Workflow]])
        failure.exception shouldBe an [WorkflowNotRunningException]
      }
    }
